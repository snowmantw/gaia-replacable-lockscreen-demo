...
(function() {
  ...
  GestureController.prototype.start = function() {
    ...
    this.cover.style.background = 'red';
    ...
    switch (evt.type) {
      case 'touchstart':
        ...
        if (evt.touches.length === 2) {
          console.log('>>>>>>> double fingers touch started');
          this.touches = evt.touches;
          this.states.touchStartX = this.touches[0].screenX;
      ...
      case 'touchmove':
        this.states.touchStartX = this.touches[0].screenX;
        var delta = evt.touches[0].screenX - this.states.touchStartX;
        if ( Math.abs(delta) > this.threshold && delta < 0) {
          // Left
          this.mainPromise.then(this.onLeft.bind(this));
        } else if (Math.abs(delta) > this.threshold && delta > 0){
          // Right
          this.mainPromise.then(this.onRight.bind(this));
        } else {
          console.log('>>> XX: no left or right moving');
        }
        break;
      case 'touchend':
  ...
  GestureController.prototype.onLeft = function() {
    ...
    window.dispatchEvent(new CustomEvent('gesture-left-lockscreen'));
    this.cover.addEventListener('touchstart', this);
    this.cover.removeEventListener('touchmove', this);
    this.cover.removeEventListener('touchend', this);
  };
  ...
  GestureController.prototype.onRight = function() {
    ...
    window.dispatchEvent(new CustomEvent('gesture-right-lockscreen'));
    this.cover.addEventListener('touchstart', this);
    this.cover.removeEventListener('touchmove', this);
    this.cover.removeEventListener('touchend', this);
  };
  ...
  ReplacableLockScreen.prototype.setupGesture = function() {
    ...
    return this._gestureController.start()
      .then(() => {
        console.log('>>>> START append constroller cover');
        this.elements.browserContainer.appendChild(this._gestureController.cover);
        console.log('>>>> DONE append constroller cover');
      })
      .catch(this.onMainPromiseError.bind(this));
  };
  ...
  ReplacableLockScreen.prototype.loadDefault = function() {
    ...
    iframe.classList.add('activated-lockscreen-content');
  ...
  ReplacableLockScreen.prototype.setupEvents = function() {
    ...
    window.addEventListener('gesture-left-lockscreen', this);
    window.addEventListener('gesture-right-lockscreen', this);
  };
  ...
  ReplacableLockScreen.prototype.handleEvent = function(e) {
    ...
    switch(e.type) {
      case 'gesture-left-lockscreen':
        this.mainPromise.then(this.onLeftLockScreen.bind(this));
        break;
      case 'gesture-right-lockscreen':
        this.mainPromise.then(this.onRightLockScreen.bind(this));
        break;
      case 'enabledstatechange':
        if (e.application.enabled) {
          this.mainPromise.then(this.onEnable.bind(this));
        } else {
          this.mainPromise.then(this.onDisable.bind(this));
        }
        break;
      case 'uninstall':
        this.mainPromise.then(this.onUninstall.bind(this));
        break;
    }
  };
